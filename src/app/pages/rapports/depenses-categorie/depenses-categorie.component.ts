...
@Component({
  selector: 'app-depenses-categorie',
  templateUrl: './depenses-categorie.component.html',
  styleUrls: ['./depenses-categorie.component.scss'],
})
export class DepensesCategorieComponent implements OnInit {
  ...
  load(seg) {
    this.db.getDatabaseState().subscribe(rdy => {
      if (rdy) {
        ...
        this.db.loadDepenseByCategorie(date, seg);
        this.db.getJournal().subscribe(data => {
          this.depenses = data;
          this.totDepense = this.depenses.map(t => t.depense).reduce((acc, value) => acc + value, 0);
        });
      }
    });
  }
  ...
  print() {
    ...
    content += 'DEPENSES PAR CATEGORIE';
    content += '</th>';
    content += '</tr>';
    content += '<tr>';
