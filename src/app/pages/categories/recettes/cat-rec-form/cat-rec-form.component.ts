...
@Component({
  selector: 'app-cat-rec-form',
  templateUrl: './cat-rec-form.component.html',
  styleUrls: ['./cat-rec-form.component.scss'],
})
export class CatRecFormComponent implements OnInit {
  ...
  onSubmit(values) {
    if (this.mode === 'new') {
      this.db.addCategorie(values.nom, values.tarif, values.color, 'R', '')
        .then(_ => {
          this.backTolist();
        });
    } else {
